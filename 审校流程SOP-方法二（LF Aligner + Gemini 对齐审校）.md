...
> 在原有 NotebookLM 逐页审校方法（`审校流程SOP.md`）之外的另一套方法：用 LF Aligner 做中英段落级对齐，用 Glossary 词汇库秒级初筛，再用 Gemini 做双语语境深度审校。
...
1. 让 Claude Code 执行终端命令，自动调用 LF Aligner 对 `EN.docx` 和 `ZH.docx` 进行段落级对齐。
...
- **对齐参数**：`--filetype="t" --segment="n" --review="x" --tmx="n"`，`--review="x"` 直接产出 xls 对照表（即 `Aligned_vX.xlsx`）。
- **⚠️ 必须用段落级对齐 `--segment="n"`，不要用句级 `"y"`**：2026-08 实测第四卷第 1 章，EN/ZH 同源于同一 Word 段落结构（段落 1:1 对应），段落级对齐正确率约 **97%**，句级仅 **47%**。句级切分会导致中英行错乱，后续审校全部建立在错误对应上。
...
3. **AI 语境审查（模拟点击调用 Gemini）**：**送审范围为全部对齐行**（分批发，每批 50 对 + 章节上下文），**不要只送词汇库预筛行**——2026-08 实测预筛 26 条候选里 25 条是同形误报，而真错（如 世俗→民事、操练→执行）反而不在词库错词列表上。词汇库预筛只用于**给候选排优先级**，不用于决定审不审。脚本将**专属的 Gemini 神学审校提示词**连同**中英对齐的段落（每批 50 对）**，通过模拟点击操作自动提交给 Gemini。等待 Gemini 问答结束后，脚本将回答内容抓取拿回来，并自行进行解析和分析。
...
| 句级对齐 `--segment="y"` | 中英行错乱，后续审校建立在错误对应上 | 用段落级 `--segment="n"`（EN/ZH 同源于同一 Word 段落，1:1 对应，实测段落级 97%） |
| 词汇库含同形误报词 | 预筛候选大量假阳性，浪费送审 | 清洗误报词（宣告/恢复/证明/誓言/会议/注释/长老会/法令），只留语境专属错译 |
| 只送预筛行给 Gemini | 真错不在词库上，漏检 | 全部对齐行分批发给 Gemini，预筛只用于排优先级 |
...
>
> **清洗原则（2026-08 实测后修订）**：Wrong_Term 只保留「在该词专有神学语境下才是错译」的词；**删除同形误报词**——在日常语境中正确、仅凭文本匹配就会大量假阳性的词（如 宣告、恢复、证明、誓言、会议、注释、长老会、法令）。实测这些词让预筛 26 条候选里 25 条是误报，而真错（如 世俗→民事、操练→执行）反而不在词库上。下表已按此原则清洗。

| 英文原词 (EN) | 常见机器错译 / 俗翻 (Wrong_Term) | 标准改革宗译法 (Suggested_Term) | 所属神学类别 (Category) |
| --- | --- | --- | --- |
| Justification | 辩护 / 开脱 / 正当理由 | 称义 | 救恩论 (Soteriology) |
| Sanctification | 批准 / 认可 / 神圣化 | 成圣 | 救恩论 |
| Regeneration | 再生 / 翻新 | 重生 | 救恩论 |
...
| Session | 开庭 / 学期 | 堂会 / 治理委员会 | 教会体制 (Presbyterian Polity) |
| Presbytery | 神父 | 区会 / 长老区会 | 教会体制 |
...
| Exegesis | 考证 | 解经 / 释经 | 释经学 |
| Eisegesis | 读入 / 主观解释 | 私意解经 / 读入式解经 | 释经学 |
| Vocation | 职业 / 培训 | 呼召 / 天职 | 伦理学 / 基督徒生活 |

### 模式化检查项（增补，2026-08）

> 词汇库只覆盖「术语」，还需补充三类**可模式化**的机器翻译错误。这三类是 2026-08 实测中词汇库完全覆盖不到、但真实存在的高发错误。建议在 `_batch_audit_vX.py` 里以正则模式实现，命中即送 Gemini 确认。

| 检查项 | 典型错误 | 检测模式（示例） |
| --- | --- | --- |
| **丛书/文献缩写** | 本丛书缩写 RST（Reformed Systematic Theology）被直译成《修订标准译本》；教父文献 NPNF/NPNF1（Nicene and Post-Nicene Fathers）被译成《新约全集》《新约圣经注释集》 | 中文侧出现「修订标准译本」「新约全集」等书名，而英文侧是 RST / NPNF 缩写；或中文直译缩写而英文侧无对应书名 |
| **语言标注** | 希伯来语词 qahal 被标成「（希腊文：qahal）」；希腊语词 ekklēsia 可能被标成「希伯来文」 | 中文出现「（希腊文：<词>）」/「（希伯来文：<词>）」，校验该词实际语系（qahal、qēhāl 属希伯来文，ekklēsia 属希腊文） |
| **章节号引用** | 英文 "in chap. 2" 被译成「第1章」这类错位 | 中文「第N章」对照英文 "chap. N" / "ch. N" 的数字是否一致 |
